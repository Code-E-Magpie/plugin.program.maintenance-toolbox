# ============================================================
#################################
# speedtest_ookla.py by Code-E-Magpie
#################################
# ============================================================

# original speedtest-cli DEV by Matt Martz (https://github.com/sivel/speedtest-cli)
# sourced from: script.ezmaintenanceplus > resources > lib > modules > speedtest.py
# location: plugin.program.maintenance-toolbox > resources > lib > tab2 > speedtest_ookla.py
# type: connection
...
ADDON_DATA_MT = xbmcvfs.translatePath('special://profile/addon_data/plugin.program.maintenance-toolbox')
ADDON_NAME = 'Maintenance Toolbox'
ADDON_TITLE = (' '.join((ADDON_NAME).strip(' '))) # insert spaces + remove leading & trailing
TEXT_ADDON = 'yellow'
TEXT_DARK = 'darkgray'
TEXT_GENERAL = 'silver'
TEXT_ITEM = 'blue'
...
# ============================================================
# Addon_Title / Dialogue / DialogueProgress / Log_Title / Speedtest
# ============================================================
...
class FakeShutdownEvent(object):
	"""Class to fake a threading. Event.is_set so that users of this module are not required to register their own threading.Event()"""
...
# Begin import to handle Python 2 and Python 3
try:
	import json
except ImportError:
	try:
...
try:
	from urllib2 import (urlopen, Request, HTTPError, URLError, AbstractHTTPHandler, ProxyHandler, HTTPDefaultErrorHandler, HTTPRedirectHandler, HTTPErrorProcessor, OpenerDirector)
except ImportError:
...
# ============================================================
	...
	def print_(*args, **kwargs):
		"""The new-style print function for Python 2.4 and 2.5. Taken from https://pypi.python.org/pypi/six/ Modified to set encoding to UTF-8 always, and to flush after write"""
...
def _build_connection(connection, source_address, timeout, context = None):
	"""Cross Python 2.4 - Python 3 callable to build an ``HTTPConnection`` or ``HTTPSConnection`` with the args we need. Called from ``http(s)_open`` methods of ``SpeedtestHTTPHandler`` or ``SpeedtestHTTPSHandler``"""
...
def build_opener(source_address = None, timeout = 10):
	"""Function similar to ``urllib2.build_opener`` that will build an ``OpenerDirector`` with the explicit handlers we want, ``source_address`` for binding, ``timeout`` and our custom `User-Agent`"""

	printer(prefix + 'Timeout set to %d' % timeout, debug = True)

	if source_address:
		source_address_tuple = (source_address, 0)
		printer(prefix + 'Binding to source address: %r' % (source_address_tuple,), debug = True)
	else:
...
class GzipDecodedResponse(GZIP_BASE):
	"""A file-like object to decode a response encoded with the gzip method, as described in RFC 1952. Largely copied from ``xmlrpclib``/``xmlrpc.client`` and modified to work for py2.4-py3"""
...
def build_user_agent():
	...
	ua_tuple = ('Mozilla/5.0', '(%s; U; %s; en-us)' % (platform.system(), platform.architecture()[0]), 'Python/%s' % platform.python_version(), '(KHTML, like Gecko)', 'speedtest-cli/%s' % __version__)
	user_agent = ' '.join(ua_tuple)
	printer(prefix + 'User-Agent: %s' % user_agent, debug = True)
	return user_agent
...
def build_request(url, data = None, headers = None, bump = '0', secure = False):
	...
	printer(prefix + '%s %s' % (('GET', 'POST')[bool(data)], final_url), debug = True)
...
# ============================================================
# FUNCTION: get_attributes_by_tag_name
# ============================================================

def get_attributes_by_tag_name(dom, tag_name):
	"""Retrieve an attribute from an XML document and return it in a consistent format. Only used with xml.dom. minidom, which is likely only to be used with python versions older than 2.5"""
	...
	def run(self):
		try:
			if (timeit.default_timer() - self.starttime) <= self.timeout:
				...
				while (not self._shutdown_event.is_set() and (timeit.default_timer() - self.starttime) <= self.timeout):
...
# ============================================================
	...
	def run(self):
		...
		try:
			if ((timeit.default_timer() - self.starttime) <= self.timeout and not self._shutdown_event.is_set()):
				try:
...
class SpeedtestResults(object):
	"""Class for holding the results of a speedtest, including: Download speed, Upload speed, Ping/Latency to test server, Data about server that the test was run against. Additionally this class can return a result data as a dictionary or CSV, as well as submit a POST of the result data to the speedtest.net API to get a share results image link."""
...
# ============================================================
	...
	def share(self):
		...
		request = build_request('://www.speedtest.net/api/api.php', data = '&'.join(api_data).encode(), headers = headers, secure = self._secure)
...
# ============================================================
	...
	def __init__(self, config = None, source_address = None, timeout = 10, secure = False, shutdown_event = None):
		...
		self.results = SpeedtestResults(client = self.config['client'], opener = self._opener, secure = secure,)
...
# ============================================================
	...
	def best(self):
		if not self._best:
			raise SpeedtestMissingBestServer('get_best_server not called or not able to determine best server')
		return self._best
...
# ============================================================
	...
	def get_servers(self, servers = None, exclude = None):
		...
		for url in urls:
			try:
				request = build_request('%s?threads=%s' % (url, self.config['threads']['download']), headers = headers,  secure=self._secure)
				uh, e = catch_request(request, opener = self._opener)
				if e:
					errors.append('%s' % e)
				...
				printer(prefix + 'Servers XML:\n%s' % ''.encode().join(serversxml), debug = True)
...
# ============================================================
	...
	def set_mini_server(self, server):
		...
		extension = re.findall('upload_?[Ee]xtension: "([^"]+)"', text.decode('UTF-8'))
		if not extension:
...
# ============================================================
	...
	def get_closest_servers(self, limit = 5):
		...
		printer(prefix + 'Closest Servers:\n%r' % self.closest, debug = True)
		return self.closest

# ============================================================
# FUNCTION: get_best_server
# ============================================================
	...
	def get_best_server(self, servers = None):
		...
		for server in servers:
			...
			for i in range(0, 3):
				...
				printer(prefix + '%s %s' % ('GET', this_latency_url), debug = True)
				...
				try:
					...
					e = get_exception()
					printer(prefix + 'ERROR: %r' % e, debug = True)
					cum.append(3600)
					continue
...
# ============================================================
		...
		def producer(q, requests, request_count):
			for i, request in enumerate(requests):
				thread = HTTPDownloader(i, request, start, self.config['length']['download'], opener = self._opener, shutdown_event = self._shutdown_event)
				thread.start()
				q.put(thread, True)
...
# ============================================================
	...
	def upload(self, callback = do_nothing, pre_allocate = True):
		...
		for i, size in enumerate(sizes):
			...
			if pre_allocate:
				data.pre_allocate()
			requests.append((build_request(self.best['url'], data, secure = self._secure), size))
...
# ============================================================
		...
		def producer(q, requests, request_count):
			for i, request in enumerate(requests[:request_count]):
				thread = HTTPUploader(i, request[0], start, request[1], self.config['length']['upload'], opener = self._opener,	 shutdown_event = self._shutdown_event)
				thread.start()
				q.put(thread, True)
				callback(i, request_count, start = True)
...
def parse_args():
	...
	parser.add_argument('--share', action = 'store_true', default = True, help = 'Generate and provide a URL to the speedtest.net share results image, not displayed with --csv')
	...
	parser.add_argument('--no-pre-allocate', dest = 'pre_allocate', action = 'store_const', default = True, const = False, help = 'Do not pre allocate upload data. Pre allocation is enabled by default to improve upload performance. To support systems with insufficient memory, use this option to avoid a MemoryError')
...
# ============================================================
# FUNCTION: validate_optional_args
# ============================================================
			...
			if timer == True:
				for i in range(0, 10):
					...
					for key, value in kwargs.items():
						buf = buf + '\n' + value
					DialogueProgress.update(count, label1 + buf)
					time.sleep(1)
				timer = False
			else:
				buf = ''
				for key, value in kwargs.items():
					buf = buf + '\n'  + value
				DialogueProgress.update(100, label1 + buf)
...
def shell():
	...
	printer(prefix + 'Retrieving speedtest.net configuration...', quiet)
	try:
		speedtest = SpeedtestObject(source_address = args.source, timeout = args.timeout, secure = args.secure)
	except (ConfigRetrievalError,) + HTTP_ERRORS:
		printer(prefix + 'Cannot retrieve speedtest configuration', error = True)
	...
	if args.list:
		try:
			...
			printer(prefix + 'Cannot retrieve speedtest server list', error = True)
			raise SpeedtestCLIError(get_exception())
	...
	printer(prefix + 'Testing from %(isp)s (%(ip)s)...' % speedtest.config['client'], quiet)

	if not args.mini:
		printer(prefix + 'Retrieving speedtest.net server list...', quiet)
		try:
			...
			raise SpeedtestCLIError('%s is an invalid server type, must be an int' % ', '.join('%s' % s for s in args.server))

		if args.server and len(args.server) == 1:
			printer(prefix + 'Retrieving information for the selected server...', quiet)
		else:
			printer(prefix + 'Selecting best server based on ping...', quiet)
	...
	printer(prefix + 'Hosted by %(sponsor)s (%(name)s) [%(d)0.2f km]: %(latency)s ms' % results.server, quiet)

	if args.download:
		printer(prefix + 'Testing download speed', quiet, end = ('', '\n')[bool(debug)], error = False)
		speedtest.download(callback = callback)
		printer(prefix + '[COLOR %s] > Download: [/COLOR][COLOR %s]%0.2f M%s/s[/COLOR]' % ( TEXT_ITEM, TEXT_VALUE, (results.download / 1000.0 / 1000.0) / args.units[1], args.units[0]), quiet)
	else:
		printer(prefix + 'Skipping download test', quiet)

	if args.upload:
		printer(prefix + 'Testing upload speed', quiet, end = ('', '\n')[bool(debug)])
		speedtest.upload(callback = callback, pre_allocate = args.pre_allocate)
		printer(prefix + '[COLOR %s] > Upload: [/COLOR][COLOR %s]%0.2f M%s/s[/COLOR]' % (TEXT_ITEM, TEXT_VALUE, (results.upload / 1000.0 / 1000.0) / args.units[1], args.units[0]), quiet)
	else:
		printer(prefix + 'Skipping upload test', quiet)

	printer(prefix + 'Results:\n%r' % results.dict(), debug = True)

	if args.simple:
		printer(prefix + 'Ping: %0.2f ms\nDownload: %0.2f M%s/s\nUpload: %0.2f M%s/s' % (results.ping, (results.download / 1000.0 / 1000.0) / args.units[1], args.units[0], (results.upload / 1000.0 / 1000.0) / args.units[1], args.units[0]))
	elif args.csv:
...
def main():
	try:
		...
		Log(Log_Title + Speedtest + '[COLOR %s][LIGHT]Finished (Speedtest by Ookla)[/LIGHT][/COLOR]' % TEXT_DARK, xbmc.LOGINFO)
